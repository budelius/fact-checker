...
This product should be built as a claim-level evidence pipeline, not as a single chatbot answer. The durable value is the owned knowledge base: Markdown notes are the canonical user-facing memory, while Qdrant and MongoDB provide retrieval and relationship views over the same evidence.
...
Use a Python/FastAPI backend, Next.js/TypeScript frontend, MongoDB for operational metadata, structured entity documents, rating snapshots, and graph relationships, Markdown files as the canonical knowledge vault, and Qdrant for vector/hybrid search. Use OpenAI Responses API for live web search, extraction, summarization, and comparison, but keep durable knowledge outside OpenAI-hosted memory.
...
- MongoDB: jobs, reports, source metadata, rating snapshots, entity documents, graph relationships, and source trails across creators, claims, papers, authors, sources, and evidence.
...
- Qdrant indexing and MongoDB graph relationship writes.
...
The system should be a job-oriented pipeline with clear adapters. User requests create jobs; workers run TikTok ingestion, transcript processing, claim extraction, source discovery, paper processing, evidence evaluation, Markdown writing, vector indexing, graph writes, and rating updates. Markdown is canonical for human-readable knowledge; MongoDB tracks operational records, structured entity documents, and relationships; Qdrant is the vector query layer with consistency checks.
...
5. Knowledge stores - Markdown vault, MongoDB, Qdrant, media/PDF storage.
...
4. **Knowledge store drift** - use stable IDs and consistency checks across Markdown, MongoDB, and Qdrant.
...
**Addresses:** Knowledge ownership, MongoDB, Qdrant, provider adapters.
...
- **Phase 1:** FastAPI, Next.js, MongoDB, Qdrant, Docker Compose patterns are well documented.
...
- **MongoDB graph modeling:** Phase 1 should define entity collections, relationship edge collections, traversal query patterns, and escape criteria for a future dedicated graph database.
